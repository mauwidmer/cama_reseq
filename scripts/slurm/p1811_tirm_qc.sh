...
#SBATCH --job-name=qc_p1811_trim
#SBATCH --partition=kenlab2
#SBATCH --nodelist=fgcz-kl-004
#SBATCH --output=./log/p1811_Cama_trim_qc.out
...
OUTDIR="/scratch/maurin/cama_reseq/data/trimmed/reseqs/p1811_qc"
...
for sample in "${SAMPLE_LIST[@]}"; do
    ...
    fastp --in1 "${INDIR}/${sample}_R1.fastq.gz" \
          ...
          --out1 "${TRIMMED_OUTDIR}/${sample}_R1_trimmed.fastq.gz" \
          --out2 "${TRIMMED_OUTDIR}/${sample}_R2_trimmed.fastq.gz" \
...
# Running FastQC on filtered data
...
for sample in "${SAMPLE_LIST[@]}"; do
    ...
    fastqc --outdir "$FASTQC_OUTDIR" \
           ...
           "${TRIMMED_OUTDIR}/${sample}_R1_trimmed.fastq.gz" "${TRIMMED_OUTDIR}/${sample}_R2_trimmed.fastq.gz"
done

# Running MultiQC
echo "starting multiqc..."
multiqc "$OUTDIR" --force --filename "Cama_p1811_trim_mqc.html"
